...
pair of records appearing again in another case, `@sanning/proof` 0.11.0
grades every one of those candidates and reports the worst.

An honest exchange of several messages in one direction lands there. Each
`handoff.sent` record now has more than one candidate `handoff.received`
record signed by the same key, and each `handoff.received` record has more
than one candidate `handoff.sent` record; every one of them reads `failed` on
`message_hash` and `sequence`, with the finding `counterpart_ambiguous`, and
the case reads `mixed`.

You can tell this apart from a genuine tamper by reading the candidates
themselves: one of them reads `verified`, and the rest are the exchange's
other honest messages, not a decoy planted beside a broken pair. A change to
this reading is under review.

## Declared records that are not hand-off sides

A case pack may declare records beside its hand-off sides: a step record, an
ordinary log line. When a pack declares a record and its own declaration
claims it as none of its hand-offs, the case reader names that record rather
than reading past it, so a case holding one does not read `verified`.
