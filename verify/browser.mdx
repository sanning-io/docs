...
It also reads a case file, the single archive `case` writes when several parties' packs
are handed over together. Drop it the same way, and the page reads it as one case.
...
| Card | Answers |
|---|---|
| Signatures | Did the agent sign each record |
| Event chain | Does each record link to the one before it |
| Completeness | Is anything missing from what the stamp covered. **Not** whether the agent showed you every run |
...
The page offers a report of what it checked. It is your own re-verification, rendered
