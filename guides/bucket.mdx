...
resolves it, see [Keep what you anchored](/guides/log-store).
...
The store takes your own client, and an `endpoint` with path-style addressing
makes an S3-compatible backend behave like S3. Object Lock support differs
between backends and changes between their releases, so this page names none of
them.

**The SDK detects and reports, which is the only claim that survives a backend
changing under you.** Run the readiness check against a real bucket on the
backend you intend to use, and read what it says before you rely on it.
