...
`path` and `content_type` join `name` and `byte_length` when the tool
supplied them. The hash lives on the record itself, in `event.content_hash`,
not in this metadata.

<Note>
  **From 0.16.0, the record commits to the file's own bytes** (proof#92).
  `event.content_hash` is the file's SHA-256, and the bytes go to your log
  store the same way any other content does: never sent to Sanning, which
  receives the signed envelope only. A record written before this release
  commits to a descriptor instead, and does not match the file when it is
  handed in beside a pack; see [Verify at the command line](/verify/cli).
</Note>
...
`event.content_hash` in the record. No Sanning code takes part in that
step:
...
## Shapes the SDK recognizes

Three shapes in a tool's return value are recognized:
