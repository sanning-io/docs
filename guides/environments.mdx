...
Neither SDK has a default. `environment` is required in both, from
`@sanning/anchor` and `sanning-anchor` 0.15.0, so a service cannot inherit the
wrong one by omission. In TypeScript, an anchorer built with no signer and no
subject in `dev` generates an identity for you and says so, loudly, on the way
past.
...
- **Production refuses an auto-generated identity.** In TypeScript, building an
  anchorer for production without an explicit signer and subject raises
  `ProductionConfigError` at construction, so a dev-mode identity cannot reach
  it by accident. Python cannot reach that state at all: `signer` and
  `subject` are required arguments.
...
1. Create the production key in the console. Every key can anchor, which is
   what a production runtime needs. [API keys and scopes](/guides/keys-and-scopes)
   covers what that covers.
...
Give the dev version of an agent its own `agent_id`, such as
...
<CardGroup cols={2}>
  ...
  <Card title="API keys and scopes" icon="key" href="/guides/keys-and-scopes">
    The key production needs, and the two things a key can do.
  </Card>
</CardGroup>
