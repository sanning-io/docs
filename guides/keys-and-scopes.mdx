...
description: "The three scopes an agent needs, and why enrollment is not a step you write"
---
...
Access to `console.sanning.io` starts with a conversation with Sanning, not a
sign-up form: [PLACEHOLDER: contact link]. Once your organization exists on
the roster, open **Settings, API keys** and select **Create key**. Name
the key, and leave **Verify** unchecked unless this key will also submit
records to a verify endpoint you run. Store it as `SANNING_API_KEY` in
whatever your service already uses for secrets. The key is a credential for
your organization, not the key that signs your evidence.

## The scopes a key needs

Every key **Anchor** grants three scopes together, and each fails at a
different moment:

- **`agent:enroll`** lets the SDK enroll an agent's signing key. It runs
...
- **`anchor:read`** lets you read your own records back, which is how a
  hand-over pack is built, and is also how the `bundle` command reports each
  pack it writes.

Checking **Verify** on top adds `verify:submit` and `verify:read`. An agent
...
- **No `agent:enroll`.** The first `anchor` call raises
  `EnrollmentFailedError`, carrying the control plane's own code and HTTP 403.
  Nothing is anchored.
