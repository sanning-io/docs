...
```ts
import { createAnchorer, FsLogStore, LocalEd25519Signer } from "@sanning/anchor";
import { anchorMiddleware } from "@sanning/anchor-vercel";
...
const anchorer = createAnchorer({
  ...
  // Any key from the console can anchor, which is everything this needs.
  apiKey,
  logStore: new FsLogStore("./logstore"),
});

const evidence = anchorMiddleware(anchorer);
...
step to write. Production refuses auto-generated secrets: `createAnchorer`
...
`run.seq`, both visible offline. Pass `chain: false` to
`anchorMiddleware` to turn the record chain off; `run.seq` is still
committed.
...
`_end` and `_error`: one event type per anchored operation, plus
`vercel_ai.run_closed`, the closing record. All seven are exported as
`EVENT_TYPES`.
