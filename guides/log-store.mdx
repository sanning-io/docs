...
- **The canonical event record**, the bytes `payload_hash` commits to. In
  TypeScript they are also on the result as `eventBytes`, in Python as
  `event_bytes`.
...
<Note>
  **From 0.16.0, the store also keeps each record's signed envelope**, next
  to the record itself under `envelopes/<payload_hash>` (proof#57). A store
  written before this release has no `envelopes/` directory and reads exactly
  as before; the layout marker stays `sanning.logstore/v1`.
</Note>
...
if (!seedHex || !apiKey) {
  throw new Error("set SANNING_SIGNING_SEED and SANNING_API_KEY");
}
...
| Flag | What it names |
|---|---|
| `--agent` | The agent's `agent_id`, as committed in each record. |
...
`subject.agent_id` lives in the record, and a minimum-disclosure envelope
...
the command exits `3` (undetermined): run it again after the interval seals.
When nothing in the window matches at all, it exits `1`.

The partial case exits `3` too, and for a different reason:
...
```ts TypeScript
import { createAnchorer, ObjectLogStore } from "@sanning/anchor";
...
<CardGroup cols={2}>
  <Card title="API keys and scopes" icon="key" href="/guides/keys-and-scopes">
    What a key carries, and why the pack command needs no extra grant.
  </Card>
